...
<div class="container">
        ...
        <div id="awardInfoButton">
            ...
            var lang_award_info_ln1 = "<?= __("JCG - Japan Century Guns Award"); ?>";
            var lang_award_info_ln2 = "<?= __("May be claimed for having contacted (heard) and received a QSL card from an amateur station located in each of at least 100 different guns (counties/districts) of Japan."); ?>";
            var lang_award_info_ln3 = "<?= __("JCG-200, 300, 400, 500 will be issued as separate awards."); ?>";
    ...
    <form class="form" action="<?php echo site_url('awards/jcg'); ?>" method="post" enctype="multipart/form-data">
        <fieldset>
            ...
            <div class="mb-3 row">
                <div class="col-md-2"><?= __("Deleted guns"); ?></div>
                <div class="col-md-10">
                    <div class="form-check-inline">
    ...
    <ul class="nav nav-tabs" id="myTab" role="tablist">
        ...
        <li class="nav-item">
            <a class="nav-link" onclick="load_jcg_map();" id="map-tab" data-bs-toggle="tab" href="#jcgmaptab" role="tab" aria-controls="home" aria-selected="false"><?= __("Map"); ?></a>
        </li>
    </ul>
    <br />

    <div class="tab-content" id="myTabContent">
        <div class="tab-pane fade" id="jcgmaptab" role="tabpanel" aria-labelledby="home-tab">
    <br />
    ...
    if ($jcg_array) {
        echo '
                <table id="jcgTable" style="width:100%" class="table-sm table table-bordered table-hover table-striped table-condensed text-center">
                    <thead>
                    <tr>
						<td>' . __("Number") . '</td>
						<td>' . __("Gun") . '</td>';
        ...
        echo '</tr>
                    ...
                    <tbody>';
        foreach ($jcg_array as $jcg => $value) {
            echo '<tr>';
            echo '<td style="text-align: center">' . $value['Number'] . '</td>';
            echo '<td style="text-align: center">' . $value['Gun'] . '</td>';
            foreach ($bands as $band) {
				echo '<td style="text-align: center">' . awards_render_jcc_cell($jcg, $band, $value[$band], $postdata) . '</td>';
            }
            echo '</tr>';
        }
        echo '</table>
        ...
        if (count($bands) > 2) {
           $len_worked = count($jcg_summary['worked']);
           $j = 0;
           foreach ($jcg_summary['worked'] as $jcg) {
               if ($j == $len_worked - 1 - $sat) {
                  echo '<td style="text-align: center"><b>' . $jcg . '</b></td>';
               } else {
                  echo '<td style="text-align: center">' . $jcg . '</td>';
               }
               $j++;
           }
        } else {
           echo '<td style="text-align: center"><b>' . $jcg_summary['worked']['Total'] . '</b></td>';
        }
        ...
        if (count($bands) > 2) {
           $len_confirmed = count($jcg_summary['confirmed']);
           $j = 0;
           foreach ($jcg_summary['confirmed'] as $jcg) {
           ...
           echo '<td style="text-align: center"><b>' . $jcg_summary['confirmed']['Total'] . '</b></td>';
        }

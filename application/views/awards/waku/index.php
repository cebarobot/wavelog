...
<div class="container">
        ...
        <div id="awardInfoButton">
            ...
            var lang_award_info_ln1 = "<?= __("WAKU - Worked All Ku's Award"); ?>";
            var lang_award_info_ln2 = "<?= __("May be claimed for having contacted (heard) and received a QSL card from an amateur station located in each of the wards (ku) of Japan's designated cities."); ?>";
    ...
    <form class="form" action="<?php echo site_url('awards/waku'); ?>" method="post" enctype="multipart/form-data">
        <fieldset>
            ...
            <div class="mb-3 row">
                ...
                <div class="col-md-10">
                    ...
                    <?php if ($waku_array) {?>
                    <button type="button" onclick="load_waku_map();" class="btn btn-info btn-sm"><i class="fas fa-globe-asia"></i> <?= __("Show WAKU Map"); ?></button>
    ...
    <ul class="nav nav-tabs" id="myTab" role="tablist">
        ...
        <li class="nav-item">
            <a class="nav-link" onclick="load_waku_map();" id="map-tab" data-bs-toggle="tab" href="#wakumaptab" role="tab" aria-controls="home" aria-selected="false"><?= __("Map"); ?></a>
        </li>
    </ul>
    <br />

    <div class="tab-content" id="myTabContent">
        <div class="tab-pane fade" id="wakumaptab" role="tabpanel" aria-labelledby="home-tab">
    <br />

    <div id="wakumap" class="map-leaflet" ></div>
    ...
    if ($waku_array) {
        echo '
                <table id="wakuTable" style="width:100%" class="table-sm table table-bordered table-hover table-striped table-condensed text-center">
                    <thead>
                    <tr>
						<td>' . __("Number") . '</td>
						<td>' . __("Ward") . '</td>';
        ...
        echo '</tr>
                    ...
                    <tbody>';
        foreach ($waku_array as $waku => $value) {
            echo '<tr>';
            echo '<td style="text-align: center">' . $value['Number'] . '</td>';
            echo '<td style="text-align: center">' . $value['Ward'] . '</td>';
            foreach ($bands as $band) {
				echo '<td style="text-align: center">' . awards_render_jcc_cell($waku, $band, $value[$band], $postdata) . '</td>';
            }
            echo '</tr>';
        }
        echo '</table>
        ...
        if (count($bands) > 2) {
           $len_worked = count($waku_summary['worked']);
           $j = 0;
           foreach ($waku_summary['worked'] as $waku) {
               if ($j == $len_worked - 1 - $sat) {
                  echo '<td style="text-align: center"><b>' . $waku . '</b></td>';
               } else {
                  echo '<td style="text-align: center">' . $waku . '</td>';
               }
               $j++;
           }
        } else {
           echo '<td style="text-align: center"><b>' . $waku_summary['worked']['Total'] . '</b></td>';
        }
        ...
        if (count($bands) > 2) {
           $len_confirmed = count($waku_summary['confirmed']);
           $j = 0;
           foreach ($waku_summary['confirmed'] as $waku) {
           ...
           echo '<td style="text-align: center"><b>' . $waku_summary['confirmed']['Total'] . '</b></td>';
        }

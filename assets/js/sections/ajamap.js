...
function load_aja_map() {
    $('.nav-tabs a[href="#ajamaptab"]').tab('show');
    $.ajax({
        url: base_url + 'index.php/awards/aja_map',
        type: 'post',
        data: {
            band: $('#band2').val(),
            mode: $('#mode').val(),
        ...
        success: function(data) {
            load_aja_map2(data, worked, confirmed, notworked);
        },
        error: function() {
...
function load_aja_map2(data, worked, confirmed, notworked) {

    // If map is already initialized
    var container = L.DomUtil.get('ajamap');

    if(container != null){
        container._leaflet_id = null;
        container.remove();
        $("#ajamaptab").append('<div id="ajamap" class="map-leaflet" ></div>');
    }

    var map = new L.Map('ajamap', {
    ...
    for (var f = 0; f < jsonFiles.length; f++) {
        $.ajax({
            dataType: "json",
            url: base_url + jsonFiles[f],
            async: false,
            success: function(result) {
                for (var item in result) {
                    var name = item.toString();
                    entityStuff[name] = [result[item]['name'], result[item]['lat'], result[item]['lon']];
                }
            }
        });
    }

    for (const [key, value] of Object.entries(entityStuff)) {
       var D = [];
       if (key in data) {
...
function onClick(e) {
    var marker = e.target;
    displayContactsOnMap($("#ajamap"), marker.options.prefix, $('#band2').val(), 'All', 'All', $('#mode').val(), 'JCC');
}

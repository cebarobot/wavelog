...
function load_waku_map() {
    $('.nav-tabs a[href="#wakumaptab"]').tab('show');
    $.ajax({
        url: base_url + 'index.php/awards/waku_map',
        type: 'post',
        data: {
            band: $('#band2').val(),
            mode: $('#mode').val(),
        ...
        success: function(data) {
            load_waku_map2(data, worked, confirmed, notworked);
        },
        error: function() {
...
function load_waku_map2(data, worked, confirmed, notworked) {

    // If map is already initialized
    var container = L.DomUtil.get('wakumap');

    if(container != null){
        container._leaflet_id = null;
        container.remove();
        $("#wakumaptab").append('<div id="wakumap" class="map-leaflet" ></div>');
    }

    var map = new L.Map('wakumap', {
    ...
    $.ajax({
       dataType: "json",
         url: base_url + 'assets/json/japan_award/ku_list.json',
       async: false,
       success: function(result) {
          for (var item in result) {
             var name = item.toString();
             wakustuff[name] = [result[item]['name'], result[item]['lat'], result[item]['lon']];
          }
       }
    });
    for (const [key, value] of Object.entries(wakustuff)) {
       var D = [];
       if (key in data) {
...
function onClick(e) {
    var marker = e.target;
    displayContactsOnMap($("#wakumap"), marker.options.prefix, $('#band2').val(), 'All', 'All', $('#mode').val(), 'JCC');
}
